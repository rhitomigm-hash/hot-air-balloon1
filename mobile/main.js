...
import {
  MAX_COLORS, DEFAULT_APPEARANCE, PATTERN_IDS, TAPE_COLORS,
  encodeAppearance, decodeAppearance, isValidAppearanceCode,
...
import { buildBalloon, buildGoreTexture, skirtColorFor } from './balloon-render.js';
...
function setTimeScale(v) {
  ...
  document.getElementById('tscale-btn').textContent = v;
}

// ---- タッチ操作ボタン(押している間だけ有効な長押し系はポインタイベントで扱う) ----
function bindHoldButton(id, onDown, onUp) {
  ...
  el.addEventListener('contextmenu', (e) => e.preventDefault()); // 長押しメニュー防止
}
bindHoldButton('btn-burner', () => { input.burner = true; }, () => { input.burner = false; });
bindHoldButton('btn-rip', () => { input.rip = true; }, () => { input.rip = false; });
document.getElementById('btn-marker').addEventListener('click', () => { if (flightReady) dropMarker(); });
document.getElementById('btn-view').addEventListener('click', toggleView);
document.getElementById('btn-speed').addEventListener('click', () => {
  const order = [1, 2, 4, 8];
  const cur = MP.room ? MP.desired : timeScale; // マルチプレイは自分の希望値を循環させる
  setTimeScale(order[(order.indexOf(cur) + 1) % order.length]);
});
...
const renderSoundBtn = () => {
  ...
  soundBtn.textContent = soundOn ? (stalled ? t('btn.soundOnStalled') : t('btn.soundOn')) : t('btn.soundOff');
};
renderSoundBtn();
...
instrumentsEl.addEventListener('click', () => {
  ...
  document.getElementById('hud-toggle').textContent = compact ? t('hud.toggleOpen') : t('hud.toggleClose');
});
...
addEventListener('pointermove', (e) => {
  if (!look.dragging || e.pointerId !== look.pid) return;
...
const endLook = (e) => { if (e.pointerId === look.pid) look.dragging = false; };
...
function toggleWindMode() {
  ...
  document.getElementById('wind-label').textContent =
    windDisplayMode === 'to' ? t('hud.windLabelTo') : t('hud.windLabel');
  renderFlightPibal();
}
...
function selectArea() {
  return new Promise((resolve) => {
    ...
    const map = setupAreaMap((lon, lat, name) => {
      sel = { lon, lat, name };
      btn.disabled = false;
      btn.textContent = t('areasel.btnSelected', { name: name || `${lat.toFixed(3)}N, ${lon.toFixed(3)}E` });
    });
...
function setupAreaMap(onSelect) {
  ...
  // ドラッグ=パン、タップ=選択、2本指ピンチ=ズーム段階変更(マウス/タッチ共通)
  let drag = null;
  const pts = new Map();
  ...
  cv.addEventListener('pointerdown', (e) => {
    ...
    if (pts.size === 1) {
      drag = { cx: e.clientX, cy: e.clientY, vx: c.x, vy: c.y, moved: false, ox: e.offsetX, oy: e.offsetY };
    } else if (pts.size === 2) {
      drag = null;
  ...
  const endPointer = (e) => {
    if (pts.has(e.pointerId) && pts.size === 1 && drag && !drag.moved) {
      const r = cssRatio();
      const [fx, fy] = toTile(drag.ox * r, drag.oy * r);
...
function setupWindEditor() {
  ...
  sel.innerHTML = WIND_PRESETS
    ...
    .join('') + `<option value="custom">${t('wind.presetCustom')}</option>`;
...
function renderEditorRows(rows) {
  document.getElementById('wind-editor').innerHTML = rows.map((r) =>
    ...
    `<td><button type="button" class="del" title="${t('wind.delRowTitle')}">×</button></td></tr>`).join('');
}
...
function copyShare(btn) {
  ...
  navigator.clipboard.writeText(shareUrl())
    .then(() => { btn.textContent = t('copy.success'); })
    .catch(() => { btn.textContent = t('copy.fail'); })
...
// values: 実際に入る値の配列、label: 値→表示文字列
...
function setupLaunchMap() {
  ...
  // ドラッグ=パン、タップ=離陸地点の選択、2本指ピンチ=ズーム(マウス/タッチ共通)
  let drag = null;
  const pts = new Map();
  ...
  cv.addEventListener('pointerdown', (e) => {
    ...
    pts.set(e.pointerId, { x: e.clientX, y: e.clientY });
    if (pts.size === 1) {
      drag = { cx: e.clientX, cy: e.clientY, vx: view.x, vz: view.z, moved: false, ox: e.offsetX, oy: e.offsetY };
    } else if (pts.size === 2) {
      drag = null;
      pinch = { d0: pinchDist(), s0: view.scale };
    }
  });
  cv.addEventListener('pointermove', (e) => {
    if (!pts.has(e.pointerId)) return;
    pts.set(e.pointerId, { x: e.clientX, y: e.clientY });
    if (pinch && pts.size === 2) {
      view.scale = THREE.MathUtils.clamp(pinch.s0 * (pinchDist() / pinch.d0), fitScale, MAX_SCALE);
      clampView();
      render();
      return;
    }
  ...
  const endPointer = (e) => {
    if (pts.has(e.pointerId) && pts.size === 1 && drag && !drag.moved) {
      const [wx, wz] = screenToWorld(drag.ox * cssRatio(), drag.oy * cssRatio());
...
function startFlight(x, z) {
  ...
  // タッチ操作ボタンを表示
  for (const id of ['btn-burner', 'btn-rip', 'btn-marker']) {
...
document.getElementById('marker-info').textContent = t('hud.markerRemaining', { n: marker.available });
...
function dropMarker() {
  ...
  document.getElementById('marker-info').textContent = t('hud.markerDropped');
...
function stepMarker(dt, rawDt) {
  ...
  document.getElementById('marker-info').textContent = t('hud.markerFalling', { m: Math.round(m.pos.y - ground) });
}
...
function onMarkerLanded(pos) {
  ...
  document.getElementById('marker-info').textContent = t('hud.markerMeasuring');
  MP.myDist = dist;
...
function showResult(dist, note) {
  ...
  subs.push(isBest ? t('result.bestNew') : t('result.bestExisting', { m: Number(prev).toFixed(1) }));
  if (MP.room) {
    subs.push(t('mp.waitingAllMeasure'));
  ...
  document.getElementById('marker-info').textContent = t('hud.markerResult', { m: dist.toFixed(1) });

  // 離陸地点がゴールに近いほど有利になるため、参考情報として直線距離を添える
  const launchInfo = document.getElementById('result-launch');
  if (launchDist !== null) {
    launchInfo.textContent = t('result.launchDist', { m: launchDist.toFixed(0) });
...
function stepClock(dt) {
  ...
  if (remaining <= 0) {
    ...
    if (!marker.state) {
      ...
      showResult(d, t('result.timeExpired'));
    }
  }
}
...
function updateHud(w) {
  ...
  hud.status.textContent = state.grounded ? t('hud.statusGrounded') : t('hud.statusFlying');
...
// other=true で「もう一方の版」(スマホ版⇔PC版)へのURLを作る。ホストのいる版と
// 招待相手が使う端末が一致するとは限らないため(例: PC版でルームを作りスマホの
// 相手を招待すると、招待URLがPC版のまま届いて操作不能になっていた)
function mpInviteUrl(code, other) {
  const server = new URLSearchParams(location.search).get('server');
  const path = other ? location.pathname.replace('/mobile/', '/prototype/') : location.pathname;
...
async function mpJoinGate(code) {
  ...
  const tryConnect = async () => {
    ...
    if (!name) { statusEl.textContent = t('mp.enterName'); return null; }
    goBtn.disabled = true;
    ...
    try {
      ...
      statusEl.innerHTML =
        `⚠ ${esc(err.message)} <a href="${location.pathname}" style="color:#9ecfff">${t('mp.soloLink')}</a>`;
      return null;
    }
  };
...
function mpBriefingInit() {
  ...
  document.getElementById('mp-copy').addEventListener('click', (e) => {
    navigator.clipboard.writeText(mpInviteUrl(MP.code))
      .then(() => { e.target.textContent = t('copy.success'); })
      .catch(() => { e.target.textContent = t('copy.fail'); })
      .finally(() => setTimeout(() => { e.target.textContent = t('mp.copyBtn'); }, 1600));
  });
  document.getElementById('mp-copy-pc').addEventListener('click', (e) => {
    navigator.clipboard.writeText(mpInviteUrl(MP.code, true))
      .then(() => { e.target.textContent = t('copy.success'); })
      .catch(() => { e.target.textContent = t('copy.fail'); })
      .finally(() => setTimeout(() => { e.target.textContent = t('mp.copyOtherBtn'); }, 1600));
  });
  ...
  document.getElementById('mp-rematch').addEventListener('click', () => {
    ...
    document.getElementById('mp-results-note').textContent = t('mp.reconnectingRematch');
    mpTryReconnect();
  });
}
...
function mpToggleReady() {
  ...
  if (!MP.ready) {
    ...
    MP.ready = true;
    btn.textContent = t('mp.readyDone');
  } else {
    MP.room.sendUnready();
    MP.ready = false;
    const d = Math.hypot(launchSel.x - TARGET_XZ.x, launchSel.z - TARGET_XZ.z);
...
function mpWireRoom() {
  ...
  room.on('close', () => {
    ...
    MP.room = null;
    // どの局面でも自動で入り直す。スマホはLINE等へのアプリ切替だけで
    // WebSocketが切れるため、切断は異常ではなく正常系として扱う。
...
function mpScheduleReconnect(attempt) {
  if (attempt >= RECONNECT_DELAYS.length) {
    // 諦めメッセージ(画面復帰時のvisibilitychangeで再試行は続く)
    mpReconnectStatus(started && !MP.resultsShown
      ? t('mp.reconnectFailedSolo')
      : t('mp.reconnectFailedReload'));
    return;
  }
  mpReconnectStatus(t('mp.reconnectingAfterClose'));
...
async function mpTryReconnect(attempt = 0) {
  ...
  try {
    ...
    if (!started && !MP.resultsShown) {
      ...
      mpReconnectStatus(t('mp.reconnected'));
...
// アプリ切替から戻った瞬間に即時再接続を試みる(バックオフ待ちをスキップ)
...
function mpRenderRoster() {
  ...
  document.getElementById('mp-roster').innerHTML = ps.map((p) =>
    ...
    `${esc(p.name)}${p.host ? ' 👑' : ''}${p.id === MP.room.id ? t('mp.self') : ''} ` +
    `${p.ready ? '✅' : '…'}</div>`).join('');
  ...
  if (MP.room.isHost) {
    ...
    startBtn.textContent = ok ? t('mp.startBtn')
      : (ps.length < 2 ? t('mp.waitingPlayers') : t('mp.waitingReady'));
  } else {
...
function mpRenderBoard() {
  ...
  el.innerHTML =
    `<div style="color:#9ecfff">${esc(t('mp.boardHeader', { code: MP.code, scale: timeScale, desired: MP.desired }))}</div>` +
    MP.room.players.map((p) =>
      `<div><span class="chip" style="background:${BALLOON_COLORS[p.color].ui}"></span>` +
      `${esc(p.name)} <span class="done">${p.connected === false ? t('mp.disconnected')
        : p.landed ? t('mp.landed') : p.dropped ? t('mp.dropped') : ''}</span></div>`)
      .join('');
}
...
function mpShowResults(rows) {
  ...
  document.getElementById('mp-results-body').innerHTML = rows.map((r, i) =>
    ...
    `${esc(r.name)}${r.left ? t('mp.resultsLeft') : ''}${MP.room && r.id === MP.room.id ? ' ★' : ''}</td>` +
    `<td class="num">${r.dist == null ? '—' : `${Number(r.dist).toFixed(1)} m`}</td></tr>`).join('');
  document.getElementById('mp-results').style.display = 'block';
  document.getElementById('mp-results-gw').textContent = groundWind ? t('result.gwOn') : t('result.gwOff');
  ...
  if (MP.room && MP.room.isHost) {
    ...
    note.textContent = MP.room ? t('mp.waitingRematch') : t('mp.reloadToRetry');
  }
}
  ...
  if (hello) {
    ...
    if (hello.dropped || hello.landed) {
      ...
      document.getElementById('marker-info').textContent =
        hello.landed ? t('mp.markerMeasuredBeforeReload') : t('mp.markerDroppedAlready');
    }
  }
  document.getElementById('mp-board').style.display = 'block';
  mpRenderBoard();
