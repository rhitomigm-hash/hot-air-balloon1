...
// items: 各要素が_tileKey(tileKeyForの戻り値)と_distKm(タイル内での取捨選択に使う
// 距離。フライトエリア中心からの距離をそのまま流用してよい)を持つ配列。
// タイルごとにallocateQuota()の配分数だけ、_distKmが近い順に選び、内部用の
// アンダースコア始まりフィールドを取り除いて返す
export function selectByTileQuota(items, totalBudget, minPerTile = 20) {
  ...
  for (const [key, group] of byTile) {
    ...
    group.sort((a, b) => a._distKm - b._distKm);
    selected.push(...group.slice(0, take));
  }
  return selected.map(({ _distKm, _tileKey, ...rest }) => rest);
}
