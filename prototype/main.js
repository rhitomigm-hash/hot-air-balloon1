...
function mpWireRoom() {
  ...
  room.on('error', (m) => mpStatus(`⚠ ${m.msg || m.code}`)); // サーバーからの生メッセージなのでi18n対象外
  room.on('close', () => {
...
function mpRenderBoard() {
  ...
  el.innerHTML =
    `<div style="color:#9ecfff">${t('mp.boardHeader', { code: MP.code, scale: timeScale, desired: MP.desired })}</div>` +
